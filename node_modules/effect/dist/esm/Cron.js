...
const CronProto = {
  ...
  [Hash.symbol]() {
    return pipe(Hash.hash(this.tz), Hash.combine(Hash.hash(this.and)), Hash.combine(Hash.array(Arr.fromIterable(this.seconds))), Hash.combine(Hash.array(Arr.fromIterable(this.minutes))), Hash.combine(Hash.array(Arr.fromIterable(this.hours))), Hash.combine(Hash.array(Arr.fromIterable(this.days))), Hash.combine(Hash.array(Arr.fromIterable(this.months))), Hash.combine(Hash.array(Arr.fromIterable(this.weekdays))), Hash.cached(this));
  },
  toString() {
 ...
 * @since 3.20.0
 */
export const prev = (cron, startFrom) => {
 ...
 * @since 3.20.0
 */
export const sequenceReverse = function* (cron, startFrom) {
  while (true) {
 ...
 * @since 2.0.0
 */
export const Equivalence = /*#__PURE__*/equivalence.make((self, that) => self.and === that.and && restrictionsEquals(self.seconds, that.seconds) && restrictionsEquals(self.minutes, that.minutes) && restrictionsEquals(self.hours, that.hours) && restrictionsEquals(self.days, that.days) && restrictionsEquals(self.months, that.months) && restrictionsEquals(self.weekdays, that.weekdays));
...
const parseSegment = (input, options) => {
  ...
  const add = wildcard && (first[1] === undefined || first[1] === 1) ? constVoid : value => {
