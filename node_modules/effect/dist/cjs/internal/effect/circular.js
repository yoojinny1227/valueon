...
class Semaphore {
  ...
  updateTakenUnsafe(fiber, f) {
    ...
    if (this.waiters.size > 0) {
      ...
      }, fiber.getFiberRef(core.currentSchedulingPriority), fiber);
    }
...
const timeoutTo = exports.timeoutTo = /*#__PURE__*/(0, _Function.dual)(2, (self, {
  ...
  onSuccess,
  onTimeout
}) => core.fiberIdWith(parentFiberId => core.uninterruptibleMask(restore => fiberRuntime.raceFibersWith(core.exit(restore(self)), core.interruptible(effect.sleep(duration)), {
  onSelfWin: (winner, loser) => core.flatMap(winner.await, exit => {
    const selfExit = core.exitFlatten(exit);
    if (selfExit._tag === "Success") {
      return core.flatMap(winner.inheritAll, () => core.as(core.interruptAsFiber(loser, parentFiberId), onSuccess(selfExit.value)));
    } else {
      return core.flatMap(core.interruptAsFiber(loser, parentFiberId), () => core.exitFailCause(selfExit.cause));
    }
  }),
