...
const redactConfigError = error => configError.reduceWithContext(error, void 0, RedactedConfigErrorReducer);
const fromFlatLoop = (flat, prefix, config, split) => {
  ...
  switch (op._tag) {
    ...
    case OpCodes.OP_REDACTED:
      {
        return core.suspend(() => (0, _Function.pipe)(fromFlatLoop(flat, prefix, op.original, split), core.mapError(redactConfigError), core.map(Arr.map(op.redact))));
      }
